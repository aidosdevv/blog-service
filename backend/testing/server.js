...
app.post('/login', loginLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(401).json({ error: 'Қате логин немесе құпия сөз' });

    const valid = await bcrypt.compare(password, user.password);
    if (!valid) return res.status(401).json({ error: 'Қате логин немесе құпия сөз' });
    ...
    console.log(err)
    res.status(500).json({ error: 'Кіру кезінде қате болды' });
  }
});

app.get('/profile', authMiddleware, async (req, res) => {
  res.json({ message: `Сәлем, ${req.user.username}! Бұл қорғалған бет.` });
});
